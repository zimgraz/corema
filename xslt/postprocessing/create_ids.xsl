...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="t:msItem">
        <xsl:variable name="abbr">
            <xsl:value-of select="substring-after(//t:idno[@type='PID'],'corema.')"/>
        </xsl:variable>
        <xsl:variable name="locus">
            <xsl:value-of select="translate(normalize-space(t:locus),' +öäü;,*:#','')"/>
        </xsl:variable>
        <xsl:variable name="num">
            <xsl:number level="any" format="1" count="t:msItem"/>
        </xsl:variable>

        <xsl:copy>
            <xsl:attribute name="xml:id">
                <xsl:value-of select="$abbr"/><xsl:text>.</xsl:text><xsl:value-of select="$locus"/><xsl:text>.</xsl:text><xsl:value-of select="$num"/>  
            </xsl:attribute>
