...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="t:body//t:ab[not(@subtype)]">
        <xsl:variable name="counter">
            <xsl:number from="t:body//t:milestone" level="any" count="t:body//t:ab[not(@subtype)]"/>
        </xsl:variable>
        <xsl:copy>
            ...
            <xsl:attribute name="xml:id">
                <xsl:value-of select="concat(lower-case(preceding-sibling::t:milestone[1][@unit='collection']/@n),'.',$counter)"/>
            </xsl:attribute>
